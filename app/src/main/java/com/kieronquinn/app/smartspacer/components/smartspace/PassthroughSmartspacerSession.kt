 ...
 *  config.
 */
abstract class PassthroughSmartspacerSession(
    ...
    abstract fun SystemSmartspaceRepository.getSystemTargets(): StateFlow<List<SmartspaceTarget>>
    abstract val surface: UiSurface
    ...
    private fun setupCollection() = whenCreated {
        systemSmartspaceRepository.getSystemTargets().collect {
            collectInto.invoke(
                sessionId,
                it.map { target ->
                    target.toSystemSmartspaceTarget(context, surface)
                }
            )
        }
    }
