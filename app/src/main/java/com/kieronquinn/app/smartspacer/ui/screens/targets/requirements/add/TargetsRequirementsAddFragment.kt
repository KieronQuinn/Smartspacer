...
import com.kieronquinn.app.smartspacer.ui.screens.targets.requirements.add.TargetsRequirementsAddViewModel.State
...
class TargetsRequirementsAddFragment: BoundFragment<FragmentRequirementsAddBinding>(FragmentRequirementsAddBinding::inflate), BackAvailable {
    ...
    private val args by navArgs<TargetsRequirementsAddFragmentArgs>()
    ...
    private val viewModel by viewModel<TargetsRequirementsAddViewModel>()

    private val adapter by lazy {
        TargetsRequirementsAddAdapter(
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        setupSearch()
        setupSearchClear()
        viewModel.setup(args.targetId, args.pageType)
    }
    ...
    private fun setupRecyclerView() = with(binding.requirementsAddRecyclerview) {
        ...
        adapter = this@TargetsRequirementsAddFragment.adapter
    ...
    private fun setupSearch() = with(binding.includeSearch) {
        ...
        val blur = BlurDelegate.get(
            BlurMode.View(searchBoxBlur, binding.requirementsAddBlurTarget, blurBackground),
            lifecycleScope
        )
        blur.setBlur(1f)
        whenResumed {
