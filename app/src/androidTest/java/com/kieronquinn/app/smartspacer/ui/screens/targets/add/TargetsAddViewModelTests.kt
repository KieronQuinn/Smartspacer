...
class TargetsAddViewModelTests: BaseTest<TargetsAddViewModel>() {
    ...
    private val mockTargets by lazy {
        listOf(
    ...
    override fun setup() {
        super.setup()
        every { targetsRepositoryMock.getAllTargets() } returns mockTargets
    ...
    fun testState() = runTest {
        ...
        sut.state.test {
            ...
            val itemToSelect = items.items.first { it is Item.Target }
            sut.onTargetClicked(itemToSelect as Item.Target)
    ...
    fun testShowWidgetPermissionDialog() = runTest {
        ...
        sut.showWidgetPermissionsDialog(grant)
        coVerify(exactly = 1) {
            navigationMock.navigate(
                TargetsAddFragmentDirections.actionTargetsAddFragmentToWidgetPermissionDialog(grant)
            )
        }
    }
    ...
    fun testShowNotificationsPermissionDialog() = runTest {
        val grant = mock<Grant>()
        sut.showNotificationPermissionsDialog(grant)
        coVerify(exactly = 1) {
            navigationMock.navigate(
                TargetsAddFragmentDirections.actionTargetsAddFragmentToNotificationPermissionDialogFragment(grant)
            )
        }
    }
