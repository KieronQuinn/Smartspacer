...
/**
 *  Basic target showing a title, subtitle and icon, of timer type. This target demonstrates
...
class TimerTarget: SmartspacerTargetProvider() {
    ...
    private fun getTargets(): List<SmartspaceTarget> {
        ...
        return listOf(
            TargetTemplate.Basic(
                id = "timer_target",
                componentName = ComponentName(BuildConfig.APPLICATION_ID, "timer"),
                featureType = SmartspaceTarget.FEATURE_TIMER,
                title = Text("Timer"),
                subtitle = Text(timer.timerValue.value),
                icon = Icon(AndroidIcon.createWithResource(BuildConfig.APPLICATION_ID, R.drawable.ic_target_timer_stopwatch)),
                onClick = TapAction(
                    pendingIntent = PendingIntent.getBroadcast(
                        provideContext(),
                        1002,
                        Intent("${BuildConfig.APPLICATION_ID}.TIMER_CLICK").apply {
                ...
                //Hide the "Timer" title on the AoD on Native Smartspace
                hideTitleOnAod = true
            }
        )
    }
    ...
    override fun onDismiss(smartspacerId: String, targetId: String): Boolean {
        isDismissed = true
        if(timer.timerRunning.value){
            //Stop running the timer now
            timer.toggleTimer(provideContext())
        }
        notifyChange()
        return true
    }
    ...
    override fun getConfig(smartspacerId: String?): Config {
        return Config(
            "Timer Target",
            "Basic target showing a title, subtitle and icon, of timer type. This target " +
